            ...
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    using (HttpResponseMessage responseContent = await client.GetAsync(resourceLocation))
                    {
                        ...
                        ConsoleHelper.WriteSuccessLine(string.Format("The top 5 invoices for customer '0002' on company '{0}' were obtained with success.", CompanyKey));
                        Console.WriteLine("");
                        foreach (SalesInvoiceResource invoice in invoices)
                        {
                            Console.WriteLine("Invoice: Company {0} - {1}.{2}.{3} (id = {4})", invoice.Company, invoice.DocumentType, invoice.Serie, invoice.SeriesNumber, invoice.InvoiceId);
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    using (HttpResponseMessage responseContent = await client.GetAsync(resourceLocation))
                    {
                        ...
                        if (responseContent.IsSuccessStatusCode)
                        {
                            ...
                            if (!silentMode)
                            {
                                ConsoleHelper.WriteSuccessLine(string.Format("The last invoice of customer '0002' on company '{0}' was found with success.", CompanyKey));
                                Console.WriteLine("");
                            }
